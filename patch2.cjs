const fs = require('fs');
const content = fs.readFileSync('./components/BudgetCalculator.tsx', 'utf-8');
...
// 3. OTP Functions 
// We will look for setupRecaptcha and remove everything up to handleDownloadPDF
const funcStart = newContent.indexOf('  const setupRecaptcha = () => {');
const funcEnd = newContent.indexOf('  const handleDownloadPDF = () => {');
if (funcStart !== -1 && funcEnd !== -1) {
  newContent = newContent.slice(0, funcStart) + newContent.slice(funcEnd);
}

// 4. Update JSX renderer block
...
const blockStartIdx = newContent.indexOf(jsxStartSearch);
const blockEndIdx = newContent.indexOf(divSearchEnd) + divSearchEnd.length;

if (blockStartIdx !== -1 && blockEndIdx !== -1) {
  // We'll replace this whole block
  ...
  newContent = newContent.slice(0, blockStartIdx) + replacementJsx + newContent.slice(blockEndIdx);
} else {
...
fs.writeFileSync('./components/BudgetCalculator.tsx', newContent);
